...
def CalcPerc (i,df,ma_short,ma_medium,ma_long):
    df['MA_' + str(ma_short)] = ta.ema(df["close"], length=ma_short)
    df['MA_' + str(ma_medium)] = ta.ema(df["close"], length=ma_medium)
    ...
    return(i,ma_short,ma_medium,ma_long,profit,drawdown)
...
if __name__ == '__main__':
    ...
    for i in range(0, len(df3.index)):
        pool.apply_async(CalcPerc, args=(i,df, df3['MA1'].iloc[i], df3['MA2'].iloc[i], df3['MA3'].iloc[i]), callback=get_result)
    pool.close()
    pool.join()
    df4 = DataFrame (results,columns=['Index','MA1','MA2','MA3','Profit','Drawdown'])
