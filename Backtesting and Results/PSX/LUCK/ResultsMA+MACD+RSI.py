...
df.ta.rsi(length=rsilength, append=True) # length=14
df.ta.macd(fast=macdfast, slow=macdslow, signal=macdsignal, append=True) # fast=12, slow=26, signal=9
df['previousfast'] = df.iloc[:,5].shift(1)
df['previousslow'] = df.iloc[:,7].shift(1)
df['MA_' + str(ma_short)] = ta.ema(df["close"], length=ma_short)
df['MA_' + str(ma_long)] = ta.ema(df["close"], length=ma_long)
df.loc[(df.iloc[:, 5] >= df.iloc[:, 7]) & (df.iloc[:, 8] <= df.iloc[:, 9]) & (df.iloc[:,5] <= df.iloc[:,6]) & (df.iloc[:, 4] <= rsilower) & (df.iloc[:, 10] >= df.iloc[:, 11]),['buy']] = 1
df.loc[(df.iloc[:, 5] <= df.iloc[:, 7]) & (df.iloc[:, 8] >= df.iloc[:, 9]) & (df.iloc[:,5] >= df.iloc[:,6]) & (df.iloc[:, 4] >= rsiupper) & (df.iloc[:, 10] <= df.iloc[:, 11]),['buy']] = 0
df = df.rename(columns={df.columns[4]: 'RSI',df.columns[5]: 'MACD Fast', df.columns[6]: 'MACD Signal', df.columns[7]: 'MACD Slow'})
...
print("RSI Config   ", rsilength,",", rsiupper,",", rsilower)
...
layout = dict(
    title='MA+MACD+RSI Strategy',
    title_x=0.5,
    legend_title="Plots",
    margin=dict(l=20, r=100 ,t=60, b=10),
    paper_bgcolor='rgba(20,24,35,1)',
    plot_bgcolor='rgba(20,24,35,1)')
fig = make_subplots(rows=3,cols=1,shared_xaxes=True,vertical_spacing = 0,row_heights=[0.7, 0.15, 0.15])
fig.add_trace(
    go.Ohlc(x=df.index,
    open=df['open'],
    high=df['high'],
...
fig.add_trace(
    ...
    y=df.iloc[:, 7],
    line=dict(color='blue'),
    name=str(df.columns[7])),2,1)
fig.add_trace(
    go.Scatter(x=df.index,
    y=df.iloc[:, 4],
    line=dict(color='deeppink'),
    name=str(df.columns[4])),3,1)
fig.add_trace(
    go.Scatter(x=df.index,
    y=np.repeat(rsiupper, len(df)),
    fill='tonexty',
    line=dict(color='purple', dash='dash'),
    name=str('RSI Upper')),3,1)
fig.add_trace(
    go.Scatter(x=df.index,
    y=np.repeat(rsilower, len(df)),
    fill='tonexty',
    line=dict(color='purple', dash='dash'),
    name=str('RSI Lower')),3,1)
fig.add_trace(
    go.Scatter(x=df.index,
    y=df['location'],
    mode="markers+text",
...
fig.add_annotation(text=
                ...
                borderwidth=2)
fig.add_annotation(text=
                "RSI " + str(rsilength) + "," + str(rsiupper) + "," + str(rsilower),
                align='left',
                showarrow=False,
                xref='paper',
                yref='paper',
...
plotly.offline.plot(fig, filename = 'ResultsMA+MACD+RSI.html', auto_open=False, config=dict(displayModeBar=True, scrollZoom = True))